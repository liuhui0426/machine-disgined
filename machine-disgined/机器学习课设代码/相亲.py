...
# 读出数据
df = pd.read_csv(r'C:\Users\sd\Desktop\机器学习\speed_dating_train.csv', encoding='utf8')
dating_test = pd.read_csv(r'C:\Users\sd\Desktop\机器学习\speed_dating_test.csv', encoding='utf8')
...
plt.subplots(figsize=(6, 6), dpi=110, )
# 构造数据
...
names = [
    'Single:' + str(single_percentage) + '%',
    'Matched' + str(matched_percentage) + '%']
# 创建饼图  绘制女生通过Speed Dating找到对象的饼图
plt.pie(
    size_of_groups,
    labels=names,
    labeldistance=1.2,
...
temp = pd.DataFrame(predict_test)
temp.index.name = "uid"
temp.columns = ["match"]
print(temp)
